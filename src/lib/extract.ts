...
function buildPrompt(asOf: string): string {
  return [
    ...
    "- Return the amount as a positive number with no symbol.",
    "- currency: the ISO 4217 code of the symbol actually shown — ₹ or Rs is",
    "  INR, $ is USD, € is EUR, £ is GBP. A UPI history is rupees, so INR is",
    "  almost always right here. Report what you see; never convert.",
...
export async function extractTransactionsFromText(
  ...
  const prompt = [
    ...
    "- currency: the ISO 4217 code of the symbol shown beside that amount --",
    "  $ is USD, ₹ or Rs is INR, € is EUR, £ is GBP. Report the symbol you can",
    "  actually see. Never convert between currencies and never guess INR",
    "  because the reader is Indian; a foreign bill is a normal thing to find.",
    ...
    "",
    "Never invent a figure that does not appear in the text, and never restate",
    "an amount in a different currency than the one printed beside it.",
    "",
    "--- EMAIL TEXT ---",
    body.slice(0, 12_000),
  ].join("\n");
